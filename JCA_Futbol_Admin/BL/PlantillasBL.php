...
if (isset($_POST['action']) && !empty($_POST['action'])) {
    ...
    switch ($action) {
        ...
        case 'generarReporteAmonestados':
            ...
            $jugadores = $dbRep->seleccionarJugadores($idCampeonato);
            ...
            $indicadorLetraFechaLlenado = 68; //Letra D
            $indicadorCeldaJugadorLlenado=4;
            for($t=0;$t<count($jugadores);$t++)
            {
                for($o=0;$o<count($fechas);$o++)
                {
                    $plantilla->getActiveSheet()->setCellValue(chr($indicadorLetraFechaLlenado) . $indicadorCeldaJugadorLlenado, "0");   
                    $indicadorLetraFechaLlenado++;
                }
                $indicadorLetraFechaLlenado = 68; 
                $indicadorCeldaJugadorLlenado++;
            }
            
            for($j=0;$j<count($jugadores);$j++)
            {
                    $datosJugador=$dbRep->datosJugadorAmonestado($jugadores[$j]['IdJugador'],$idCampeonato);
                    $plantilla->getActiveSheet()->setCellValue(chr($indicadorLetraJugador) . $indicadorCeldaJugador, $numeroJugador);
                        $numeroJugador = $numeroJugador + 1;
                        $indicadorLetraJugador = $indicadorLetraJugador + 1;
                        $plantilla->getActiveSheet()->setCellValue(chr($indicadorLetraJugador) . $indicadorCeldaJugador, $datosJugador[0]['Equipo']);
                        $indicadorLetraJugador = $indicadorLetraJugador + 1;
                        $plantilla->getActiveSheet()->setCellValue(chr($indicadorLetraJugador) . $indicadorCeldaJugador, $datosJugador[0]['NombreJugador']);
                       $indicadorLetraJugador = 65;          
                       
                       $datosTarjetasJugador=$dbRep->reporteTarjetas($idCampeonato, $jugadores[$j]['IdJugador']);
                       
                       for($h=0;$h<count($datosTarjetasJugador);$h++)
                       {
                           for($f=0;$f<count($fechas);$f++)
                           {
                               if($datosTarjetasJugador[$h]['IdFecha']==$fechas[$f]['idfecha'])
                               {
                                   $indicadorLetraFecha=$indicadorLetraFecha+($f);
//                                   echo "\nLETRA FECHA¨: ".$indicadorLetraFecha." IDJUGADOR:".$jugadores[$h]['IdJugador']." \n";
//                                   echo "F:".$f."\n";
